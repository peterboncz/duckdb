		...
		if constexpr (HasClusteredLocalState<OP, STATE_TYPE>::value) {
			...
			case VectorType::FOR_VECTOR: // hook 4: accumulate straight from the narrow payload
				if constexpr (std::is_integral<INPUT_TYPE>::value) {
					auto &validity = input.Buffer().GetValidityMask();
					const auto data = FlatVector::GetDataUnsafe(input);
					ForVector::MarkExploited(input);
					switch (GetTypeIdSize(ForVector::StoredType(input))) {
					case 1:
						ExecuteUnaryClusteredDispatch<STATE_TYPE, INPUT_TYPE, OP>(
						    reinterpret_cast<const uint8_t *>(data), clustered, validity);
						return;
					case 2:
						ExecuteUnaryClusteredDispatch<STATE_TYPE, INPUT_TYPE, OP>(
						    reinterpret_cast<const uint16_t *>(data), clustered, validity);
						return;
					default:
						ExecuteUnaryClusteredDispatch<STATE_TYPE, INPUT_TYPE, OP>(
						    reinterpret_cast<const uint32_t *>(data), clustered, validity);
						return;
					}
				}
