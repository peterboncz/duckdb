...
public:
	...
	inline static void UnPackBuffer(data_ptr_t dst, data_ptr_t src, idx_t count, bitpacking_width_t width,
		...
		if (!NumericLimits<T>::IsSigned() || skip_sign_extension || width == 0 || width == sizeof(T) * 8) {
			UnPackBufferBulk<T>(dst, src, count, width);
			return;
		}
		for (idx_t i = 0; i < count; i += BITPACKING_ALGORITHM_GROUP_SIZE) {
...
private:
	...
	static inline void UnPackBufferBulk(data_ptr_t dst, data_ptr_t src, idx_t count, bitpacking_width_t width) {
		...
		if (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value) {
			duckdb_fastpforlib::fastunpack_bulk<uint8_t>(reinterpret_cast<const uint32_t *>(src),
			                                             ...
			                                             static_cast<size_t>(count), static_cast<uint32_t>(width));
		} else if (std::is_same<T, int16_t>::value || std::is_same<T, uint16_t>::value) {
	...
	static inline void UnPackGroup(data_ptr_t dst, data_ptr_t src, bitpacking_width_t width,
	                               bool skip_sign_extension = false) {
		if (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value) {
			duckdb_fastpforlib::fastunpack(reinterpret_cast<const uint8_t *>(src),
			                               reinterpret_cast<uint8_t *>(dst), static_cast<uint32_t>(width));
		} else if (std::is_same<T, int16_t>::value || std::is_same<T, uint16_t>::value) {
			duckdb_fastpforlib::fastunpack(reinterpret_cast<const uint16_t *>(src),
			                               reinterpret_cast<uint16_t *>(dst), static_cast<uint32_t>(width));
		} else if (std::is_same<T, int32_t>::value || std::is_same<T, uint32_t>::value) {
			duckdb_fastpforlib::fastunpack(reinterpret_cast<const uint32_t *>(src),
			                               reinterpret_cast<uint32_t *>(dst), static_cast<uint32_t>(width));
		} else if (std::is_same<T, int64_t>::value || std::is_same<T, uint64_t>::value) {
