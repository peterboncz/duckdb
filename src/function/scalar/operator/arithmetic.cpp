...
unique_ptr<BaseStatistics> PropagateNumericStats(ClientContext &context, FunctionStatisticsInput &input) {
	...
	if (potential_overflow) {
		...
		expr.function.SetFunctionCallback(GetFORArithmeticFunction<BASEOP>(expr.GetReturnType().InternalType()));
		expr.function.SetErrorMode(FunctionErrors::CANNOT_ERROR);
	}
